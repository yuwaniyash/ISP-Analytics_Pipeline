...
.stSelectbox select { cursor: pointer !important; }
...
with c3:
    y_min = df_usage["avg_gb_used"].min() * 0.95  # a little padding below
    y_max = df_usage["avg_gb_used"].max() * 1.05  # a little padding above
    ...
    fig3.update_layout(
        **DARK,
        yaxis=dict(range=[y_min, y_max])  # always use full dataset range
    )
