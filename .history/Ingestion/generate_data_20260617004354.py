...
for customer in customers:
    for day in range(NUM_DAYS):
        ...
        usage = round(random.uniform(0.5, limit * 1.1), 2)
        usage_logs.append({
...
with open(f'{OUTPUT_DIR}/usage_logs.csv', 'w', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=usage_logs[0].keys())
    writer.writeheader()
    writer.writerows(usage_logs)

print(f"✅ usage_logs.csv — {len(usage_logs)} rows")
...
for i in range(1, 201):
    ...
    tickets.append({
        ...
        'sla_breached': 1 if resolved_days > 3 else 0
    })
